...
class _AddEditSportScreenState extends State<AddEditSportScreen> {
  ...
  File? _sportImage; // new selected sport image
  File? _bannerImage; // new selected banner image
  File? _webImage; // new selected web banner

  String _status = 'AVAILABLE';
  bool _isCompressing = false;

  bool get _isEdit => widget.sport != null;
  ...
  void _saveSport() async {
    ...
    // ADD MODE: all images must be selected
    if (!_isEdit) {
      if (_sportImage == null || _bannerImage == null || _webImage == null) {
        Messenger.alertError('Please select all images');
        return;
      }
    }
    ...
    CustomConfirmationDialog.show(
      context: context,
      title: _isEdit ? 'Update Sport' : 'Add Sport',
      message: _isEdit ? 'Update this sport?' : 'Add this sport?',
      icon: _isEdit ? Icons.edit : Icons.add,
      iconColor: _isEdit ? Colors.orange : Colors.green,
      confirmColor: AppColors.bluePrimaryDual,
      onConfirm: () async {
        final provider = context.read<AddSportsProvider>();
        ...
        try {
          if (!_isEdit) {
            ...
            // EDIT
            // Only new images are sent; null images mean "no change"
            await provider.updateSport(
              id: widget.sport!['_id'].toString(),
              name: data['name'].toString(),
              about: data['about'].toString(),
  ...
  Widget _buildImagePicker({
    ...
    final hasNetwork = networkImage != null && networkImage.trim().isNotEmpty;
  ...
  @override
  Widget build(BuildContext context) {
    final isLoading = context.watch<AddSportsProvider>().isLoading;

    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: AppBar(
        centerTitle: false,
        title: Text(_isEdit ? 'Edit Sport' : 'Add Sport'),
        backgroundColor: AppColors.bluePrimaryDual,
        foregroundColor: Colors.white,
      ),
      body: AbsorbPointer(
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: (_isCompressing || isLoading)
                          ? null
                          : _saveSport,
                      style: ElevatedButton.styleFrom(
                        backgroundColor: AppColors.bluePrimaryDual,
                      ...
                      child: (_isCompressing || isLoading)
                          ? const Row(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                CircularProgressIndicator(color: Colors.white),
                          ...
                          : Text(
                              _isEdit ? 'Update Sport' : 'Add Sport',
                              style: const TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.bold,
                                color: Colors.white,
